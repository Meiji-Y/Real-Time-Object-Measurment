...
while True:
    ...
    imgContours, conts = utlis.getContours(img,draw=True, showCanny=True, minArea=50000, filter=4)
    
    if len(conts) != 0:
        biggest = conts[0][2]
        #print(biggest)
        imgWarp = utlis.warpImg(img,biggest,wP,hP)
        imgContours2, conts2 = utlis.getContours(imgWarp, minArea=1000, filter=4, cThr=[50,50],draw=False)
        if len(conts) != 0:
            for obj in conts2:
